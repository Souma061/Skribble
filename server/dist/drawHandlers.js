...
export function getTotalCachedStrokesCount() {
    let count = 0;
...
export function registerDrawHandlers(io, socket) {
    ...
    socket.on("draw:start", (payload) => {
        if (!rateLimit(socket.id, "draw:start", 60, 60_000))
            return;
        ...
        const color = typeof payload.color === "string" && HEX_COLOR_RE.test(payload.color)
            ? payload.color
            : "#2E1065";
        const size = typeof payload.size === "number" && payload.size >= 1 && payload.size <= 50
            ? payload.size
            : 6;
        const startPoint = {
    ...
    socket.on("draw:chunk", (payload) => {
        if (!rateLimit(socket.id, "draw:chunk", 30, 1_000))
            return;
        const roomId = socket.data.roomId;
        if (!roomId)
            return;
        ...
        if (isBinaryPayload(payload)) {
            ...
            if (points.length === 0)
                return;
            const byteLength = payload instanceof ArrayBuffer
                ? payload.byteLength
    ...
    socket.on("draw:clear", () => {
        if (!rateLimit(socket.id, "draw:clear", 10, 60_000))
            return;
        const roomId = socket.data.roomId;
        if (!roomId)
            return;
    ...
    socket.on("draw:undo", () => {
        if (!rateLimit(socket.id, "draw:undo", 10, 60_000))
            return;
        const roomId = socket.data.roomId;
        if (!roomId)
            return;
    ...
    socket.on("draw:request-sync", () => {
        if (!rateLimit(socket.id, "draw:request-sync", 10, 60_000))
            return;
        const roomId = socket.data.roomId;
        if (!roomId)
            return;
