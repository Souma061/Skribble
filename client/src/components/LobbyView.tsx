...
import type { PlayerRole, RoomSummary } from "../types";
...
const STATUS_LABEL: Record<RoomSummary["status"], string> = {
  WAITING: "Lobby",
  WORD_SELECTION: "Starting",
  ACTIVE_ROUND: "Drawing",
  ROUND_ENDING: "Round over",
  COMPLETED: "Finished",
};
      ...
      <div className="w-full max-w-2xl bg-white rounded-3xl p-6 border border-[#E9E4F7] pastel-card space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="p-2 rounded-xl bg-[#F3E8FF] hover:bg-[#E9D5FF] text-[#6B21A8] transition-colors"
        ...
        {rooms.length === 0 ? (
          <p className="text-sm font-semibold text-[#9CA3AF] text-center py-4">
            No live rooms right now — create one above and friends will see it here!
          </p>
        ) : (
          <div className="space-y-2 max-h-72 overflow-y-auto">
            {rooms.map((room) => (
              <div
                key={room.id}
                className="p-3 rounded-2xl border-2 border-[#F3F4F6] hover:border-[#DDD6FE] flex items-center justify-between gap-3 transition-colors"
              >
                <div className="min-w-0">
                  <div className="text-sm font-extrabold text-[#1F2937] truncate">
                    {room.name}
                  </div>
                  <div className="text-xs font-bold text-[#6B7280] flex items-center gap-2">
                    ...
                    >
                      ...
                      {room.status === "ACTIVE_ROUND" ? ` • R${room.roundNumber}` : ""}
                    </span>
                    <span>
                      🎨 {room.activePlayerCount}/15
                    </span>
                ...
                <button
                  ...
                  onClick={() => {
                    onClearError();
                    onJoinRoom(username.trim(), room.id, joinRole);
                  }}
                  className="shrink-0 px-4 py-2 rounded-xl bg-[#059669] hover:bg-[#047857] disabled:bg-[#D1D5DB] disabled:cursor-not-allowed text-white text-xs font-extrabold flex items-center gap-1.5 transition-colors"
                >
        ...
        {!isUsernameValid && rooms.length > 0 && (
          <p className="text-xs font-semibold text-[#9CA3AF] text-center">
            Enter a valid display name above to join a room.
          </p>
        )}
      </div>
